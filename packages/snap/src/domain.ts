 ...
 */
export const resolveDomainToAddress = async (domainName: string): Promise<string | null> => {
  try {
 ...
 */
export const getDomainRecord = async (domainName: string): Promise<string | null> => {
  try {
    // Extract the domain name without TLD if needed
    const domain = extractDomainName(domainName);
